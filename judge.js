...
const execute = function (
  ...
  // console.log(`docker run --rm -v="${filename}":/${codefileName}/ -v="${testfileName}":/testcase.txt/ -v="${outputfileName}":/output.txt/ -v="${timeMemoryfileName}":/timeMemory.txt/ judge ${language} output.txt timeMemory.txt ${problem.time} ${problem.memory}`);

  return `docker run --rm -v="${filename}":/${codefileName}/ -v="${testfileName}":/testcase.txt/ -v="${outputfileName}":/output.txt/ -v="${timeMemoryfileName}":/timeMemory.txt/ judge ${language} output.txt timeMemory.txt ${problem.time} ${problem.memory}`;
};
const test = (problem,submission, op, callback)=>{
  ...
  let allTestcases = problem.tests;
  ...
  async.waterfall([
    ...
    function (err,next){
      if (err) next(null, err);
      async.forEachLimit( allTestcases, 1, function(curTestcase, cb){
        async.waterfall([
          function (next) {
            if (err) next(null, err);
